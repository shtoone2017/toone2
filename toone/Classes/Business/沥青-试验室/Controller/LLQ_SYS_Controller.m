...
@implementation LLQ_SYS_Controller
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary * dict =@{@"biaoshiid":model.bsid,
                           @"departType":departType//[UserDefaultsSetting_SW shareSetting].userType//userType
                           };
...
- (IBAction)searchButtonClick:(UIButton *)sender {
    switch (sender.tag) {
        ...
        default:
            FuncLog;//组织机构代码块
            [self performSegueWithIdentifier:@"SW_ZZJG_Controller3" sender:nil];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([vc isKindOfClass:[LLQ_SYS_InnerController class]]) {
        LLQ_SYS_InnerController * controller = vc;
