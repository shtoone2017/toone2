...
@interface LLQ_CBCZ_Detail_ShenPi_Controller ()
...
@implementation LLQ_CBCZ_Detail_ShenPi_Controller
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"审批";
    self.txt1.placeholder = @"必填项，填写业主意见...";
}
...
- (IBAction)commitClick:(UIButton *)sender{
    ...
    NSString * urlString = appLqChaobiaoShenpi;
    NSDictionary * dic = @{@"jieguobianhao":self.jieguobianhao,
                           @"yezhuyijian":self.txt1.text,
                           @"confirmdate":[TimeTools timeStampWithTimeString:[TimeTools currentTime]],
                           @"shenpiren":[UserDefaultsSetting_SW shareSetting].userFullName,
                           };
